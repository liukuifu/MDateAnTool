    ...
    public class DBConnect
    {
        ...
        public void UpdateGo20UserInfo(string strInputDate, string strTableName, string strUITableName)
        {
            try
            {
                ...
                string qs = "SELECT * FROM " + strUITableName + " where udate = '" + strInputDate + "%' and [locale] is null";
                //1.SqlConnection
        ...
        internal void UpdateGo30UserInfoByCondition(string strInputDate, string strTableName, string strUITableName)
        {
            {
                ...
                try
                {
                    ...
                    connsd.Close();
                    DataTable dt = new DataTable();
                    string qs = "SELECT * FROM " + strUITableName + " where md5 is null";// + " where Convert(varchar,udate,120) LIKE '" + strInputDate + "%' and [locale] is null";
                    //1.SqlConnection
                    SqlConnection conn = ConnectionOpen();
                    //2.SqlCommand
                    using (SqlCommand cmd = new SqlCommand(qs, conn))
                            ...
                            if (ds != null && ds.Tables["guit"] != null && ds.Tables["guit"].Rows.Count > 0)
                            {
                                ...
                                for (int i = 0; i <= ds.Tables["guit"].Rows.Count - 1; i++)
                                {
                                    ...
                                    if (foundRows != null && foundRows.Length > 0)
                                    {
                                        ...
                                        if (strMd5 != ds.Tables["guit"].Rows[i]["md5"].ToString())
                                        {
                                            try
                                            {
